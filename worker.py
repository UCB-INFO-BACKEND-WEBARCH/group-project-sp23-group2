...
def call_census():
    #Expected data is a json body: {'county':'str','state':"str"}
    #Extract county and state from data passed in
    #Reformat string for census api
    data = request.get_json()
    county = data['county'].lower().lstrip().rstrip()
    state = data['state'].lower().lstrip().rstrip().capitalize()
